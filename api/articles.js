...
router.post("/", auth, async (req, res) => {
  ...
  try {
    ...
    const old_article = await Article.findOne({
      googleDocId: fields.googleDocId,
    });
    if (old_article) {
      return res.status(400).json({
        error: { msg: "An article for that google document already exists." },
      });
    }
    //Parse document here
    ...
    if (err._message == "article validation failed") {
      ...
      return res.json({
        error: { msg: "Article fields missing", missing: missing_fields },
      });
    }
...
router.get("/digest", async (req, res) => {
  ...
  try {
    ...
    var popular_articles = await Article.find({ isFeatured: false })
      .sort({ clicks: -1 })
      .limit(10);
